...
class Shelter(Base):
    ...
    has a with one-to-many relationship with Pet
    """
...
class Pet(Base):
    ...
    has a with many-to-one relationship to Breed and Shelter 
    """
    __tablename__ = 'pet'
...
if __name__ == "__main__":
    ...
    log.info("Creating shelters")
    nycpo = Shelter(name="NYC Pet Orphanage", website = "http://www.nyc-pet-orphanage.com")
    noph = Shelter(name = "New Orleans Pet Hotel", website = "http://www.neworleanspethotel.com")
    # add them to the session and commit to save to db
    log.info("Adding shelters to session and committing.")    
    db_session.add_all( [nycpo, noph] )
    db_session.commit()
    ...
    db_session.commit() 
    log.info("Created pets named {} and {}".format(sue.name, thomas.name))     
    
    # Finally, print a log message that indicates how many pets, breeds, and species 
    # we have in the database   
    log.info("There are currently {} pets, {} breeds, and {} species in the db".format( \
